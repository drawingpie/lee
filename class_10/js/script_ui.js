...
function init() {
    //풀페이지 셋팅
    $(".fullpage").fullpage({
        sectionsColor: ["", "#eaeef2", "#b6e4fe", "#e2dcd4", "#fff"],//배경색설정
    ...
    $("#section0  .link-wrap .link-unit .btn").mouseover(function () {
        var _index = $(this).parent().index();//순서찾기(index)
        $("#section0 .bg-wrap .bg").removeClass("on");//기존 on클래스 초기화
        $("#section0 .bg-wrap .bg").eq(_index).addClass("on");//같은 순번의 배경 on 클래스를 추가
    });
    ...
    $("#section1 .link-wrap .link-unit .btn").mouseout(function () {
        ...
        $("#section2 .link-wrap .link-unit .btn").mouseover(function(){
            ...
            setId = setInterval(checkPos, 10);//카운팅 효과를 위한 setInterval를 호출하고 변수에 담음
        });
        ...
        $("#section2 .wave-wrap .unit").on("transitionend", function(){ //setInterval를 멈추기 위한 애니메이션 끝시점 체크(transition 움직임이 끝날때)
            clearInterval(setId);//setInterval를 clear처리
        });
        ...
        $("#section3 .photo-wrap > ul ").slick({ //slick jquery 플러그인
            ...
            centerMode:true//기본 왼쪽 정렬이지만 가운데 정렬로 변경시켜줌
        }).on("afterChange",  function (event, slick, current) { //슬라이드가 한번 이동시 발생되는 이벤트 (current인자: 순번(index))
            console.log(current);
            $("#section3 .txt-wrap > ul >li").removeClass("on");
            $("#section3 .txt-wrap > ul >li").eq(current).addClass("on");
        })
        ...
        ////contact
        $("#section4  .input-wrap input[type=text]").on("textInput", function(event){//인풋에 키프레스 이벤트
            ...
            //appendTo : 추가객체.appendTo(추가될대상).추가이벤트1,추가이멘트2 ...
